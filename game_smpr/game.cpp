...
// Возможно игроков передавать не надо, т.к. мы их уже передали через gameoptions
    ...
    , ui(new Ui::game)
{
    ...
    // (!) Можно ещё сюда добавить подсветку палочек, в зависимости от числа.
    // Это, наверное, уже в отдельный метод event или типа того.
...
// Дописать выход из игры
// Вывод какого-то сообщения и возврат на стартовый экран
...
void game::removeObjectsFromScene()
{
    ...
    // (!!!!) Добавить проверку, что если палочек не осталось, то какой-то игрок выиграл. Вывести сообщение об этом и закрыть окно.
}
